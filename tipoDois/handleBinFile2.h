    ...
    while (recuperar_registro(excluidos, &registroExcluido) && achou == 0)
    {
        ...
        if (tam_reg == tam_reg_excluido){
            ...
            while (recuperar_registro(arquivo, &registroAux) && achou == 0){
                ...
                posicao += sizeof(int) + sizeof(int) + sizeof(int) + sizeof(int) + strlen(registroAux.last_n) + sizeof(int) + strlen(registroAux.first_n) + sizeof(int) + strlen(registroAux.address) + sizeof(int) + strlen(registroAux.city) + sizeof(int) + strlen(registroAux.state) + sizeof(int) + strlen(registroAux.zip) + sizeof(int) + strlen(registroAux.phone);
    ...
    if (arquivo != NULL)
    {
        while (recuperar_registro(arquivo, &registro))
        {
            ...
            while (recuperar_registro(excluidos, &registroExcluido)){
                if (registro.key == registroExcluido.key) naoprinta=1;
            }
